...
The package also includes high-performance **GPU/CUDA acceleration** via NVIDIA CUDA. By setting ``device='cuda'``, ``gplearn`` utilizes a custom virtual machine interpreter on the GPU to achieve **2x–4x speedups** on massive datasets (>500,000 samples) and large populations.
